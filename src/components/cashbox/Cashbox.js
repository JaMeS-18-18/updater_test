...
function Cashbox() {
	...
	const [tabs, setTabs] = useState([{ 'id': 1, 'active': true, 'time': getHHmm() }])
		...
		<>
			<div className="cashbox-tabs d-flex">
				<ul className="p-0 m-0">
						...
						<li className="me-1" key={item.id}>
							<div className="tab-block grow-animation">
								<div onClick={() => setActiveTab(item.id)}>
									<span className={"me-2 p-1 tab-number fz12 " + (item.active ? 'bg-primary' : 'bg-e0')}>
										{'0' + (index + 1)}
									</span>
					...
					<button className="btn-primary no-border" onClick={duplicate} tabIndex="-1">+</button>
				</ul>
			...
			<div className="tab-content card-background">
				{tabs.map((item) => (
					<section key={item.id} style={item.active ? { 'display': 'block' } : { 'display': 'none' }}>
						<Tab tabId={item.id} activeTabId={activeTabId} />
					</section>
				))}
			</div>

			{/* CONFIRM DELETE ONE PRODUCT MODAL */}
			<Modal show={showConfirmModalDeleteItem.bool}
				animation={false}
				centered
